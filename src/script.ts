...
const numberChecker = (text:string) => {
...
function mazeControllerSettings2D(mazeController: MazeController2D, sliderElement: SliderElement) {
...
function mazeControllerSettings3D(mazeController: MazeController3D, dimensionalInput: TextInput[]) {
    ...
    dimensionalInput.forEach((element) => {
        element.callbackFn = () => {
            const newValue: [number, number, number] = [parseInt(dimensionalInput[0].currentValue),parseInt(dimensionalInput[1].currentValue),parseInt(dimensionalInput[2].currentValue)]

            mazeController.mazeModel.generateNewMaze('Empty Maze', newValue.map((value) => value%2 === 0 ? value+1 : value) as [number, number, number])
        }
    })
}